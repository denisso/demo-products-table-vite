...
Одностраничное приложение (SPA), реализующее форму входа, защищённую
маршрутизацию и отображение каталога товаров с поддержкой пагинации, поиска и
сортировки.
...
- **Обработка ошибок API:**
  - HTTP **400** (некорректные учетные данные) — ошибки отображаются под
    соответствующими полями;
  - прочие ошибки — отображается уведомление (toast) о сетевом сбое.

Для тестирования можно использовать пользователей из документации DummyJSON Auth
API (например: `emilys` / `emilyspass`).
...
Имя пользователя хранится только в памяти и не сохраняется между сессиями без
токена.
...
- **Поиск:**
  ...
  - при наличии поискового запроса используется endpoint
    `/products/search?q=...`;
...
Исходный код расположен в директории `src/` и организован в соответствии с
принципами Feature-Sliced Design:
